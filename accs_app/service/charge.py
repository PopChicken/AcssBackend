...
CHARGING_COST_PER_KWH_MEDIUM = Decimal('0.70')  # 平时: 7:00~10:00 & 15:00~18:00 & 21:00~23:00
...
def calc_cost(begin_time: datetime,
    ...
    while not ((end_time - current_time).days == 0 and end_interval == current_interval):  # 当起始和结束不在同一天的同一区间时
        ...
        next_interval_time = datetime(current_time.year,current_time.month,current_time.day,BILLING_INTERVALS_END[current_interval],0,0)
        ...
        if current_interval % 7 == 0:
            ...
            if (end_time - current_time).days > 0 or end_time.day > current_time.day:
                next_interval_time = datetime(current_time.year,current_time.month,current_time.day+1,next_interval_time.hour,0,0)  # 进入下一天
        intervals_time_cnt[current_interval_type] +=(next_interval_time - current_time).seconds # 累计时间差
        current_time = next_interval_time

    print(datetime.now(),'current_time',current_time,sep='\t')
    print(datetime.now(),'end_time',end_time,sep='\t')
    current_interval_type = WHICH_TYPE[current_interval]            # 累计同一天同一区间内当前时刻到结束的时间
    intervals_time_cnt[current_interval_type] += (end_time - current_time).seconds
    ...
    intervals_charging_cost[TOP] = amount * CHARGING_COST_PER_KWH_TOP * intervals_time_cnt[TOP] / intervals_time_sum
    intervals_charging_cost[MEDIUM] =  amount * CHARGING_COST_PER_KWH_MEDIUM * intervals_time_cnt[MEDIUM] / intervals_time_sum
    intervals_charging_cost[BOTTOM] = amount * CHARGING_COST_PER_KWH_BOTTOM * intervals_time_cnt[BOTTOM] / intervals_time_sum
    print(datetime.now(),'intervals_charging_cost',intervals_charging_cost,sep='\t')
